...
abstract class AbstractBackingElemTest extends FunSuite {
  ...
  test("testComments") {
    assertResult(Set("Some linkbase")) {
      docElem.findAllElemsOrSelf.flatMap(e => getChildren(e).collect({ case c: Nodes.Comment => c }).map(_.text.trim)).toSet
    }
  }

  test("testProcessingInstructions") {
    assertResult(Set(("PITarget", "PIContent"))) {
      docElem.findAllElemsOrSelf.flatMap(e => getChildren(e).collect({ case pi: Nodes.ProcessingInstruction => pi }).map(pi => (pi.target, pi.data))).toSet
    }
  }
}
