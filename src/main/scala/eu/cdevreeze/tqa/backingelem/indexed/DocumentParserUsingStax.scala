 ...
 *
 * THIS IS THE SAME DocumentParserUsingStax AS IN YAIDOM, BUT WITH A WORKAROUND FOR A Woodstox BUG, THAT CAUSED A BUG IN THIS PARSER.
 *
...
final class DocumentParserUsingStax(
  ...
  def parse(inputSource: InputSource): Document = {
    ...
    try {
      // The workaround for the Woodstox bug that it is too restrictive about the Source that can be passed to method createXMLEventReader
