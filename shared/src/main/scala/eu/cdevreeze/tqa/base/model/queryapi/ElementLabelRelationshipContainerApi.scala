 ...
 */
trait ElementLabelRelationshipContainerApi {

  // Finding and filtering relationships without looking at source element

  def findAllElementLabelRelationships: immutable.IndexedSeq[ElementLabelRelationship]
  ...
  /**
   * Finds all element-label relationships that are outgoing from the given node.
   */
  def findAllOutgoingElementLabelRelationships(
    source: Node): immutable.IndexedSeq[ElementLabelRelationship]

  /**
   * Filters element-label relationships that are outgoing from the given node.
   */
