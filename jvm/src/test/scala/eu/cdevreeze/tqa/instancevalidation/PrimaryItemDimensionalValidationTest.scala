...
/**
 * Dimensional instance validation test case 203 (203-PrimaryItemDimensionallyInvalidError), from the XBRL Dimensions conformance suite.
 *
 * @author Chris de Vreeze
 */
@RunWith(classOf[JUnitRunner])
class PrimaryItemDimensionalValidationTest extends FunSuite {
  ...
  private val testCaseFile =
    new File(classOf[PrimaryItemDimensionalValidationTest].getResource(
      "/conf-suite-dim/200-xbrldie/203-PrimaryItemDimensionallyInvalidError/203-TestCase-PrimaryItemDimensionallyInvalidError.xml").toURI).
      ensuring(_.isFile)
  ...
  private def skipTestVariation(id: String): Boolean = {
    // I disagree with test variation V-227 and its error result.
    // A dimensional validator should not concern itself with non-dimensional segment/scenario content.

    Set("V-227").contains(id)
  }
  ...
  final def performTestVariation(
    ...
    val instance = makeTestInstance(
      "200-xbrldie/203-PrimaryItemDimensionallyInvalidError/" + instanceFileName)
