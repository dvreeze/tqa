...
class PrimaryItemDimensionalValidation270Test extends FunSuite {
  ...
  private def makeTestInstance(relativeDocPath: String): XbrlInstance = {
    val rootDir = new File(classOf[PrimaryItemDimensionalValidation270Test].getResource("/conf-suite-dim").toURI)
