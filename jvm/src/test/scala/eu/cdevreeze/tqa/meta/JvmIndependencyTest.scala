...
class JvmIndependencyTest extends FunSuite {
  ...
  private def testJvmIndependency(sources: immutable.IndexedSeq[Source]): Unit = {
    ...
    val otherImporters =
      importers.filterNot(isJavaImporter).filterNot(isScalaImporter).
        filterNot(isScalaJsDomImporter).filterNot(isYaidomImporter).filterNot(isTqaImporter)
  ...
  private def isOtherAllowedImporter(importer: Importer): Boolean = {
    ...
    val allowedFirstTermNames =
      List("ResolvedLocatorOrResource", "Namespaces", "FormulaAxis", "Axis", "Aspect", "XbrliElem", "ENames").map(n => Term.Name(n).structure)

    allowedFirstTermNames.contains(termNames.head.structure) ||
      (importedNames.map(_.names.map(_.structure)) ==
        List(List(Term.Name("eu"), Term.Name("cdevreeze"), Name.Indeterminate("tqa")).map(_.structure)))
  }
