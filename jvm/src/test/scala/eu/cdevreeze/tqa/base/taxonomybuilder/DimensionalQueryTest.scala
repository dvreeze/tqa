...
class DimensionalQueryTest extends AnyFunSuite {
  ...
  private val docBuilder: SaxonDocumentBuilder = {
    ...
    val zipFile = new File(classOf[DimensionalQueryTest].getResource("/xdt-conf-cr4-2009-10-06.zip").toURI)
    ...
    val xbrlCatalog =
      SimpleCatalog(
        ...
        Vector(
          SimpleCatalog
            .UriRewrite("http://www.xbrl.org/", otherRootDir.toURI.toString.stripSuffix("/") + "/www.xbrl.org/"),
          SimpleCatalog
            .UriRewrite("http://www.w3.org/", otherRootDir.toURI.toString.stripSuffix("/") + "/www.w3.org/")
        )
      )
    ...
    val catalog =
      SimpleCatalog(None, Vector(SimpleCatalog.UriRewrite(dummyUriPrefix.toString, "")))
